...
import com.mjz.model.SelectedCourse;
...
public class SelectedCourseDao extends BaseDao {
	...
	public boolean isSelected(int studentId,int courseId){
		boolean ret = false;
		String sql = "select * from s_selected_duty where student_id = " + studentId + " and course_id = " + courseId;
	...
	public boolean addSelectedCourse(SelectedCourse selectedCourse){
		String sql = "insert into s_selected_duty values(null,"+selectedCourse.getStudentId()+","+selectedCourse.getCourseId()+")";
		return update(sql);
	}
	/**
	 * 删除所选班
