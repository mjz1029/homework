...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"Staff1Servlet?method=Staff1List&t="+new Date().getTime(),
	        idField:'id', 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'qq',title:'QQ',width:150},
 		       	{field:'department_id',title:'班级',width:150, 
 		        	formatter: function(value,row,index){
 						if (row.departmentId){
 							var departmentList = $("#departmentList").combobox("getData");
 							for(var i=0;i<departmentList.length;i++ ){
 								//console.log(departmentList[i]);
 								if(row.departmentId == departmentList[i].id)return departmentList[i].name;
 							}
	        ...
	        onBeforeLoad : function(){
	        	try{
	        		$("#departmentList").combobox("getData")
	        	}catch(err){
	        		preLoadDepartment();
	        	}
	        }
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "将删除与教师相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "Staff1Servlet?method=DeleteStaff1",
							data: {ids: ids},
	    ...
	    function preLoadDepartment(){
	  		$("#departmentList").combobox({
		  		...
		  		editable: false, //不可编辑
		  		method: "post",
		  		url: "DepartmentServlet?method=getDepartmentList&t="+new Date().getTime()+"&from=combox",
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							var data = {departmentid:departmentid, name:name,sex:sex,mobile:phone,qq:qq,password:password};
							
							$.ajax({
								type: "post",
								url: "Staff1Servlet?method=AddStaff1",
								data: data,
								success: function(msg){
									if(msg == "success"){
	  ...
	  //下拉框通用属性
	  	$("#edit_departmentList, #add_departmentList").combobox({
	  		width: "200",
	  		height: "30",
	  		valueField: "id",
	  		textField: "name",
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							var data = {id:id, departmentid:departmentid, name:name,sex:sex,mobile:phone,qq:qq};
							
							$.ajax({
								type: "post",
								url: "Staff1Servlet?method=EditStaff1",
								data: data,
								success: function(msg){
									if(msg == "success"){
	  	...
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('load',{
	  			staff1Name: $('#search_staff_name').val(),
	  			departmentid: $("#departmentList").combobox('getValue') == '' ? 0 : $("#departmentList").combobox('getValue')
	  		});
	  	});
	});
...
<body>
	...
	<div id="toolbar">
		...
		<div style="float: left;margin-top:4px;" class="datagrid-btn-separator" >&nbsp;&nbsp;姓名：<input id="search_staff_name" class="easyui-textbox" name="search_staff_name" /></div>
		<div style="margin-left: 10px;margin-top:4px;" >班级：<input id="departmentList" class="easyui-textbox" name="department" />
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			...
	    			<td colspan="3">
	    				<input id="add_departmentList" style="width: 200px; height: 30px;" class="easyui-textbox" name="departmentid" />
	    			</td>
    	...
    	<form id="editForm" method="post">
	    	<table id="editTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			<td style="width:40px">班级:</td>
	    			<td colspan="3">
	    				<input id="edit_departmentList" style="width: 200px; height: 30px;" class="easyui-textbox" name="departmentid" />
	    			</td>
	    			<td style="width:80px"></td>
	    		</tr>
	    		<tr>
